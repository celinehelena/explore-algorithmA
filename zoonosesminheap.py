...
Original file is located at
    https://colab.research.google.com/drive/1ms0hQett5Q7ov_w1jZVPpmyHirnLYQBt
"""

!pip install osmnx networkx codecarbon
...
import heapq # Importação para o Min Heap

# Ignorar alguns avisos futuros do OSMnx que não são críticos
warnings.filterwarnings("ignore", category=FutureWarning)

# --- INÍCIO DA IMPLEMENTAÇÃO MANUAL DE DIJKSTRA ---
def dijkstra_com_min_heap(graph, no_origem, no_destino, weight='length'):
    """
    Calcula o caminho mais curto usando o algoritmo de Dijkstra com um Min Heap.
    """
    pq = [(0, no_origem)]
    ...
    while pq:
        ...
        if distancia_atual > distancias[no_atual]:
            continue
        if no_atual not in graph:
            continue

        for vizinho, dados_aresta in graph[no_atual].items():
            peso_aresta = dados_aresta.get(weight, 1)
            distancia_nova = distancia_atual + peso_aresta
            if distancia_nova < distancias[vizinho]:
                distancias[vizinho] = distancia_nova
                heapq.heappush(pq, (distancia_nova, vizinho))

    return float('inf')
# --- FIM DA IMPLEMENTAÇÃO MANUAL DE DIJKSTRA ---


def create_distance_matrix(graph, all_nodes):
    """
    Usa a implementação manual de Dijkstra com Min Heap para pré-calcular a distância.
    """
    dist_matrix = {}
    print(f"Pré-calculando a matriz de distâncias com Dijkstra (manual + Min Heap) para {len(all_nodes)} nós...")
    node_pairs = [(u, v) for i, u in enumerate(all_nodes) for v in all_nodes[i:]]

    for u, v in node_pairs:
        # Chamando nossa própria implementação de Dijkstra
        distance = dijkstra_com_min_heap(graph, u, v, weight='length')
...
# ... (O restante da função main e do código continua exatamente o mesmo) ...
